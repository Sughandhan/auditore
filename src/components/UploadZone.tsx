...
export default function UploadZone({ onParsed, onError, disabled }: UploadZoneProps) {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setState("done");
      onParsed(data.text, data.pageCount, data.fileName);
    } catch (err) {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-emerald-950/40 border border-emerald-700/40">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-emerald-500">{pageCount} pages extracted — ready to analyze</p>
        </div>
        <button
          onClick={reset}
      ...
      >
        {state === "uploading" ? (
          <>
            ...
            <p className="text-xs text-slate-500">Extracting text from all pages</p>
          </>
        ) : (
          <>
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Drag & drop or click — PDF up to 100MB
              </p>
            </div>
